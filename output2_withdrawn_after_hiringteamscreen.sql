...
select jobCapability, jobLocation, experienceFlag, idx, cleanedAnalyticalStatus, count(distinct uniqueId) numApplicants from
(
select *, 
case when nextStatus like '%withdrawn%' then 1 
      when nextStatus2 like '%withdrawn%' then 1 
...
group by jobCapability, jobLocation, experienceFlag, cleanedAnalyticalStatus,  idx 
order by jobCapability, jobLocation, experienceFlag, idx 
...
-- 3. total number of withdrawn applicants
WithdrawnApplicants_total AS (
select jobCapability, jobLocation,  count(distinct uniqueId) totalWithdrawnApplicants from analyticalTable where cleanedAnalyticalStatus like '%withdrawn%' 
group by jobCapability, jobLocation
),
...
-- 2. last funnel before withdrawn
select jobCapability, jobLocation, idx, cleanedAnalyticalStatus, count(distinct uniqueId) numApplicants from
(
select *, 
case when nextStatus like '%withdrawn%' then 1 
      when nextStatus2 like '%withdrawn%' then 1 
...
from analyticalTable) b where withdrawnFlag = 1 and nextStatus like '%withdrawn%'
group by jobCapability, jobLocation, cleanedAnalyticalStatus,  idx 
order by jobCapability, jobLocation, idx 
...
-- 3. total number of withdrawn applicants
WithdrawnApplicants_total AS (
select jobCapability, experienceFlag,  count(distinct uniqueId) totalWithdrawnApplicants from analyticalTable where cleanedAnalyticalStatus like '%withdrawn%' 
group by jobCapability, experienceFlag
),
