...
JoinData AS (
select distinct candidateId, jobId, uniqueId, jobCapability, jobLevel, jobLocation, year, week from AnalysisTable1 
...
(
select candidateId, jobId, uniqueId, jobCapability, jobLevel, jobLocation, startYear year, startWeek week, sum(distinct outFlag) totalOut from 
(
select e.candidateId, e.jobId, e.uniqueId, e.jobCapability, e.jobLevel, e.jobLocation, 
       e.year currentYear, 
       a2.year startYear,
       a2.week startWeek,
       e.week currentWeek, 
...
group by candidateId, jobId, uniqueId, jobCapability, jobLevel, jobLocation, startYear, startWeek
) o
where totalOut = 0),
...
SELECT distinct uniqueId, candidateId, jobId, jobCapability, jobLevel, jobLocation, year, week FROM
(
select e.uniqueId, e.candidateId, e.jobId, e.jobCapability, e.jobLevel, e.jobLocation, a3.year, a3.week, sum(case when funnel is NULL then 1 else 0 end) inFlag from
...
group by e.uniqueId, e.candidateId, e.jobId, e.jobCapability, e.jobLevel, e.jobLocation, a3.year, a3.week --startWeek
) a4 
where a4.inFlag > 0),
